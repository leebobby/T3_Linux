...
void rear_ADAS_setting::change_line_value(int value)
{
    ...
    if(sender==ui->left_right_Slider){
        qDebug()<<"横滑块变化";
        this->horizontal_values=2*value;
        ui->VerticelLine->move(70+this->horizontal_values,20);
        this->model=2;
//        update();
    }else if(sender==ui->high_low_Slider){
        qDebug()<<"竖滑块变化";
        this->vertical_values=2*value;
        this->model=1;
        ui->HorizontalLine->move(60,140-this->vertical_values);
//        update();
    }else{
        qDebug()<<"无信号检测到";
    }
